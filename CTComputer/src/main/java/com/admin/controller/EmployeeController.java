...
import com.admin.validator.ContractValidator;
import com.admin.validator.EmployeeValidator;
import com.model.*;
import com.other.SendMail;
...
public class EmployeeController extends BasicController {
		...
		try {
			...
			if (result.hasErrors()) {
				...
				SendMail.sendMail(employee.getEmail(), "Xin Chào, Thành Viên: " + employee.getNameEmployee(),
						"Bạn đã là thành viên chính thức là nhân viên của cửa hàng CT Computer Shop" + "\nĐịa Chỉ Email: "
	...
	public String formReset(ModelMap model, @PathVariable(value = "id") String id,
		...
		employee.setPassword("computershop" + new Date().getDate() + new Date().getMonth() + new Date().getYear());
		employeeService.editEmployee(employee);
		SendMail.sendMail(employee.getEmail(), "Xin Chào, Thành Viên: " + employee.getNameEmployee(),
