...
public class BasicController {
	...
	public String adminHome(ModelMap model, HttpServletRequest request) {
		Session session = sessionFactory.getCurrentSession();
		String hql = "SELECT computer.employee.id, computer.employee.nameEmployee, computer.employee.photo, "
				+ "SUM(CASE WHEN computer.type = 1 THEN 1 ELSE 0 END), SUM(CASE WHEN computer.type = 0 THEN 1 ELSE 0 END),"
				+ "SUM(CASE WHEN type = 1 THEN 1 ELSE 0 END) - SUM(CASE WHEN type = 0 THEN 1 ELSE 0 END),"
				+ "SUM(computer.amount) AS Tongket "
				+ "FROM Bonus computer GROUP BY computer.employee.id, computer.employee.nameEmployee, computer.employee.photo "
