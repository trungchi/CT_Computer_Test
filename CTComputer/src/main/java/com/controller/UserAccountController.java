...
public class UserAccountController extends BasicController {
		...
		try {
			...
			if (email.equals(user.getEmail())) {
				try {
					...
					helper.setFrom("CT Computer Shop <computerfc@gmail.com>");
					helper.setTo(email);
	...
	public String register(ModelMap model, @ModelAttribute("register") Customer customer,
		...
		Session session = sessionFactory.getCurrentSession();
		try {
			Customer idCustomer = customerService.getIDCustomer(customer.getId());
			if(idCustomer != null){
			...
			try {
				...
				helper.setFrom("CT Computer Shop <17h1010001chi@ou.edu.vn>");
				helper.setTo(customer.getEmail());
				helper.setReplyTo("CT Computer Shop <17h1010001chi@ou.edu.vn>");
				...
				String text = "Xin chào: " + customer.getFullName()
						+ "<br> Bạn đã đăng ký thành công tài khoản tại CT Computer Shop"
